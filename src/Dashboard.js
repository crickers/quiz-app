...
function Dashboard() {
  ...
  const fetchCategories = async () => {
    ...
    let res = await fetch(
      `${apiUrl}/api/v1/categories?token=${localStorage.getItem("token")}`
    );
  ...
  const fetchUserId = async () => {
    let userRes = await fetch(
      `${apiUrl}/api/v1/users/me?token=${localStorage.getItem("token")}`
    );
  ...
  const fetchQuestionsForCategory = async (id) => {
    ...
    let res = await fetch(
      `http://localhost:3000/api/v1/categories/${id}/questions?token=${token}&userId=${userId}`
    );
    let data = await res.json();
    console.log(data);
    data = data.reverse();
  ...
  const createNewQuestion = async () => {
    ...
    let res = await fetch(
      `${apiUrl}/api/v1/categories/${selectedCategory}/questions?token=${token}`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <div className="grid grid-cols-12">
        <div className={"col-span-full border p-5"}>
          <h1 className={"text-center text-3xl"}>
            Questions App &nbsp;&nbsp;<Button onClick={logout}>Log out</Button>
          </h1>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12">
        ...
        <div
          className={"col-span-full md:col-span-9 lg:col-span-10 border p-5"}
        >
          ...
          {selectedCategory && (
            <div>
              <input
                ...
                type="text"
                className={"border p-1 mr-5 w-2/3"}
              />
              <Button type={"primary"} onClick={createNewQuestion}>
                Create new question
              </Button>
              <br />
              <br />
                        ...
                        footer={
                          <div>
                            <input
                              ...
                              className={"border p-1 mr-5 w-2/3"}
                            />
                            <Button type={"primary"} onClick={createANewAnswer}>
                              Add Answer
                            </Button>
                          </div>
                        }
          ...
          {!selectedCategory && (
            <h1
              className={
                "text-center text-xl uppercase tracking-wider text-blue-500"
              }
            >
